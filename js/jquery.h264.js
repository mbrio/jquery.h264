...
(function($) {
	// Create private variables that represent all strings
	...
	$.h264 = {
		...
		useVideoTag: function() {
			if (!$.h264.cache_[res.useVideoTag]) {
				var obj = document.createElement(res.videoElementName);
				$.h264.cache_[res.useVideoTag] = !!(typeof(obj.canPlayType) !== 'undefined' && obj.canPlayType(res.h264Type));
			}
			
			return $.h264.cache_[res.useVideoTag];
		},
		cache_: {}
	};
	...
	var supportsCustomControls = function() {
		if (supportsCustomControls.cache_ === null) {
			supportsCustomControls.cache_ = navigator.userAgent.search(res.customControlQuery) === -1;
		}
	...
	var initControls_ = function(params) {
		this.hasControls = supportsCustomControls() && (this.controls = this.element.find(res.videoControlsSelector)).size() > 0;
